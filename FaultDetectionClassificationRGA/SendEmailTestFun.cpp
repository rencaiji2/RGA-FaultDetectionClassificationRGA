#include "SendEmailTestFun.h"
#include <QDebug>
#include <QHostInfo>
#include <QDateTime>
#include <QTextCodec>

SendEmailTestFun::SendEmailTestFun(QObject* parent)
    : QObject(parent)
    ...
    , m_boundary(generateBoundary())
{
    ...
    m_config.port = 25;
    m_config.username = "test1@ydmail.com";
    m_config.password = "123456";
...
void SendEmailTestFun::testSendWithTable()
{
...
void SendEmailTestFun::testSendWithImages()
{
    EmailContent content;
...
void SendEmailTestFun::testSendDebug()
{
    qDebug() << "=== 邮件发送测试开始 ===";
...
void SendEmailTestFun::testSend()
{
    qDebug() << "开始测试邮件发送...";
    ...
    content.to = "test2@ydmail.com";
    content.cc << "testsend@ydmail.com";
...
// 发送增强版邮件（支持HTML、附件等）
bool SendEmailTestFun::sendEnhancedEmail(const EmailContent& content)
{
    // 发送MAIL FROM命令
...
// 准备邮件数据（支持多部分MIME）
QByteArray SendEmailTestFun::prepareEmailData(const EmailContent& content)
{
...
// 创建HTML表格
QString SendEmailTestFun::createHtmlTable(const QList<QStringList>& tableData, const QStringList& headers)
{
...
// 嵌入图片（返回HTML img标签）
QString SendEmailTestFun::embedImage(const QString& imagePath)
{
...
// 创建带样式的HTML
QString SendEmailTestFun::createStyledHtml(const QString& content, const QString& css)
{
...
// 生成MIME边界
QString SendEmailTestFun::generateBoundary()
{
    return "boundary_" + QUuid::createUuid().toString().remove("{").remove("}");
}

// 以下函数保持不变...
bool SendEmailTestFun::connectToServer()
{
...
bool SendEmailTestFun::authenticate()
{
    // 发送EHLO命令
...
void SendEmailTestFun::disconnectFromServer()
{
...
QString SendEmailTestFun::encodeBase64(const QString& input)
{
    return input.toUtf8().toBase64();
}

bool SendEmailTestFun::waitForResponse(int timeoutMs)
{
